...
export default function CenturionOrders() {
  const [centurionSales, setCenturionSales] = useState([])
  const [startDate, setStartDate] = useState('')
  const [endDate, setEndDate] = useState('')

  useEffect(() => {
    const fetchCenturionSales = async () => {
  ...
  const exportCSV = () => {
    const csvData = centurionSales.map(sale => ({
    ...
    link.setAttribute('download', 'centurion_orders.csv');
