...
export default function WilsonsOrders() {
  ...
  const [showOnlyWilsons, setShowOnlyWilsons] = useState(false);

  useEffect(() => {
    const fetchWilsonsSales = async () => {
      if (startDate && endDate) {
        const { data, error } = await supabase
          .from('wilsons_orders_with_matches_mv')
          .select(`
            id,
            barcode,
            order_date,
        ...
        if (error) {
          ...
          const groupedData = data.reduce((acc, sale) => {
            ...
            if (!acc[barcode]) {
              acc[barcode] = { barcode, product, description, nett_price, quantity_sold: 0, home_hardware_price: sale.home_hardware_price, home_hardware_actual_price: sale.home_hardware_actual_price, toolbank_price: sale.toolbank_price, stax_price: sale.stax_price, matched_suppliers: sale.matched_suppliers };
            }
  ...
  const exportCSV = () => {
    const filteredData = showOnlyWilsons
      ? wilsonsSales.filter(sale => !sale.matched_suppliers)
      : wilsonsSales;
    ...
    <div style={{ padding: '2rem' }}>
      ...
      <div>
        ...
        <label style={{ marginLeft: '1rem' }}>
          <input
            type="checkbox"
            checked={showOnlyWilsons}
            onChange={(e) => setShowOnlyWilsons(e.target.checked)}
          />
          Show only Wilsons-only products
        </label>
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '1rem' }}>
          <thead>
            <tr>
              ...
              <th style={th}>Stax Price</th>
              <th style={th}>Matched Suppliers</th>
            </tr>
          </thead>
          <tbody>
            {wilsonsSales
              .filter(sale => !showOnlyWilsons || !sale.matched_suppliers)
              .map((sale) => (
                <tr key={sale.barcode}>
                  ...
                  <td style={td}>{sale.home_hardware_actual_price != null ? `£${parseFloat(sale.home_hardware_actual_price).toFixed(2)}` : ''}</td>
                  <td style={td}>{sale.toolbank_price != null ? `£${parseFloat(sale.toolbank_price).toFixed(2)}` : ''}</td>
                  <td style={td}>{sale.stax_price != null ? `£${parseFloat(sale.stax_price).toFixed(2)}` : ''}</td>
                  <td style={td}>{sale.matched_suppliers}</td>
                </tr>
              ))}
          </tbody>
        </table>
